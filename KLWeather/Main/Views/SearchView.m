...
typedef NS_ENUM(NSUInteger, ESearchView) {
...
- (void)buildView {
    ...
    CGFloat ScreenWidthForAddMoreCity   = [@"Add More City" widthWithLabelFont:font];
    TapTitleView *addMoreCity     = [[TapTitleView alloc] initWithFrame:CGRectMake(0, ScreenHeight - ScreenWidth / 2.f,
                                                                                   ScreenWidthForAddMoreCity + 60, 60)];
    addMoreCity.title = @"Add More City";
    addMoreCity.delegate = self;
    addMoreCity.font = font;
    [self addSubview:addMoreCity];
    addMoreCity.x    = ScreenWidth - addMoreCity.width;
    addMoreCity.y   += 10.f;
    
    
    // 当前位置
    CGFloat ScreenWidthCurrentLocation  = [@"Current Location" widthWithLabelFont:font];
    TapTitleView *currentLocation = [[TapTitleView alloc] initWithFrame:CGRectMake(0, ScreenHeight - 85,
                                                                                   ScreenWidthCurrentLocation + 60, 60)];
    ...
    currentLocation.x        = ScreenWidth - currentLocation.width;
    ...
    // 灰色的线条
    UIView *line1         = [[UIView alloc] initWithFrame:CGRectMake(0, ScreenHeight - ScreenWidth * 1.5, ScreenWidth, 0.5f)];
    line1.alpha           = 0.1f;
    line1.backgroundColor = [UIColor blackColor];
    [self addSubview:line1];
    
    UIView *line2          = [[UIView alloc] initWithFrame:CGRectMake(0, ScreenHeight - ScreenWidth / 2.f, ScreenWidth, 0.5f)];
    line2.alpha            = 0.1f;
    ...
    // 黑色的view
    UIView *blackView         = [[UIView alloc] initWithFrame:CGRectMake(ScreenWidth - 5, 0, 5, ScreenHeight)];
 ...
 */
- (void)initTableView {
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, ScreenHeight - ScreenWidth * 1.5, ScreenWidth, ScreenWidth)
...
- (CGFloat)tableView:(UITableView *)tableView ScreenHeightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return ScreenWidth / 4.f;
}
